...
export default function HomeClient({ initialMarkets }: HomeClientProps) {
  ...
  const [activeStatus, setActiveStatus] = useState<PredictionStatus | "ALL">(
    "ALL",
  );
  ...
  const stats = {
    ...
    success: initialMarkets.filter(
      (m) => m.status === PredictionStatus.SUCCESSFUL,
    ).length,
    precision: Math.round(
      (initialMarkets.filter((m) => m.status === PredictionStatus.SUCCESSFUL)
        .length /
        (initialMarkets.filter(
          (m) =>
  ...
  const successfulMarkets = initialMarkets.filter(
    (m) => m.status === PredictionStatus.SUCCESSFUL,
  );
        ...
        if (markets.length > 0) {
          const totalPnl = markets.reduce((sum: number, market: any) => {
            const latestSnapshot =
              market.snapshots[market.snapshots.length - 1];
  ...
  const convertAPIDataToChartFormat = (apiData: any) => {
    ...
    const chartData = sortedTimestamps.map((timestamp) => {
      ...
      markets.forEach((market: any, index: number) => {
        const snapshot = market.snapshots.find(
  ...
  const profitData = useMemo(() => {
    if (
      brushStartIndex >= 0 &&
      brushEndIndex >= brushStartIndex &&
      brushEndIndex < allFilteredData.length
    ) {
    ...
    return (
      <div className="w-full px-4 pb-32 relative">
        ...
        <div className="space-y-3 mb-8">
          ...
          <StatCard label="Precision" value="80%" />
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {marketTitles.length > 0 && (
              <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-lg">
                <div
                  className={`w-2 h-2 rounded-full ${totalReturn > 0 ? "bg-primary" : "bg-red-500"}`}
                ></div>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-white">
                ...
                {activeStatus !== "ALL" &&
                  STATUS_LABELS[activeStatus as PredictionStatus]}
                {activeCategory === "ALL" &&
                  activeStatus === "ALL" &&
                  "Filters"}
              </span>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {Array.from(
                      new Set(initialMarkets.map((m) => m.category)),
                    ).map((category) => (
                      <button
                        key={category}
                        onClick={() => {
          ...
          {filteredMarkets.length > 0 ? (
            ...
            <div className="py-24 text-center">
              <p className="text-muted font-bold tracking-widest uppercase text-xs">
                Zero results in this query vector.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mt-16 mb-24">
          ...
          <div ref={verifiedCardRef} className="glass-panel rounded-xl p-4">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <span
                ref={ribbonTriggerRef}
                className="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider border border-primary/30 bg-primary/5 text-primary"
              >
                Featured
              </span>
            </div>
            ...
            <p className="text-xs text-white/70 mb-4 line-clamp-3">
              Comprehensive competitive analysis across five dimensions
              evaluating the nomination probability and market impact of three
              candidates.
            </p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 pb-40 relative">
          ...
          <div className="mb-3 flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-white/60 font-gothic">
                Individual bet performance tracked over time with daily and
                cumulative profit indicators
              </p>
            </div>
            <div className="flex items-center gap-3">
              {marketTitles.length > 0 && (
                <div className="flex items-center gap-1.5 p-2 bg-white/5 rounded-lg">
                  <div
                    className={`w-3 h-3 rounded-full ${totalReturn > 0 ? "bg-primary" : "bg-red-500"}`}
                  ></div>
      ...
      <div
        id="market-grid"
        className="flex flex-col lg:flex-row items-center justify-between gap-6 mb-12 py-3 border-y border-white/5"
      >
        ...
        <div className="flex items-center gap-4">
          <div className="relative" ref={statusDropdownRef}>
            ...
            >
              ...
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform ${isStatusDropdownOpen ? "rotate-180" : ""}`}
              />
            </button>
      ...
      <div className="mt-32">
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div
            ref={verifiedCardRef}
            className="group cursor-pointer glass-panel spotlight-card rounded-xl transition-all duration-300 hover:border-white/20 p-6 ribbon-container relative overflow-hidden"
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  ...
                  <span
                    ref={ribbonTriggerRef}
                    className="px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border border-primary/30 bg-primary/5 text-primary group-hover:text-red-400 group-hover:border-red-400/30 group-hover:bg-red-400/5 transition-colors ribbon-trigger"
                  >
                    Featured Case
                  </span>
                </div>
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
              <div className="flex items-center gap-3">
                <div className="text-xs text-white/50">End: Dec 31, 2026</div>
                <div className="text-xs text-white/50">
                  <span className="font-medium">Vol.:</span> $155,679,905
                </div>
              </div>
            </div>
          </div>
